...
def test_validate_ldap_search_filter_valid():
    ...
    validate_ldap_search_filter(
        "(&(objectClass=person)({username_full_name_attribute}=*{username}*))"
    )
...
def test_validate_ldap_search_filter_missing_username_placeholder():
    ...
    with pytest.raises(ValueError, match="does not use the \\{username\\} placeholder"):
...
def test_validate_ldap_search_filter_invalid_syntax():
    ...
    with pytest.raises(ValueError, match="not a valid LDAP filter"):
        validate_ldap_search_filter("(objectClass=*")
